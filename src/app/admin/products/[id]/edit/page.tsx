...
export default function EditProductPage({ params }: { params: { id: string } }) {
      ...
      .then(([catData, prodData]) => {
        ...
        if (prodData.product) {
          ...
          setFormData({
            ...
            stock: p.stock ? String(p.stock) : '15',
            ...
            giftPick: Boolean(p.giftPick),
          });

          if (p.images && p.images.length > 0) {
            const extraImages = p.images
              .map((img: any) => img.url)
              .filter((url: string) => url && url !== p.thumbnail);
          ...
          if (p.variants && p.variants.length > 0) {
            setVariants(
              p.variants.map((v: any) => ({
                id: v.id,
                name: v.name,
                colorHex: v.colorHex || '#7E22CE',
                imageUrl: v.imageUrl || '',
                priceAdjust: String(v.priceAdjust || 0),
              }))
            );
          }

          if (p.customizationOpts && p.customizationOpts.length > 0) {
            setCustomizations(
              p.customizationOpts.map((c: any) => ({
                label: c.label,
                type: c.type || 'text',
                required: Boolean(c.required),
                additionalFee: String(c.additionalFee || 0),
              }))
            );
          }
        }
      })
      .catch((e) => setError('Failed to load product details.'))
  ...
  const addVariantField = () => {
    setVariants([
      ...variants,
      { name: 'Custom Color', colorHex: '#FF6B00', imageUrl: '', priceAdjust: '0' },
    ]);
  };
  ...
  if (loading) {
    return (
      <div className="p-10 text-center flex flex-col items-center justify-center min-h-[50vh]">
        <div className="w-8 h-8 border-4 border-brand-700 border-t-transparent rounded-full animate-spin mb-2" />
        <span className="text-xs text-charcoal-600 font-medium">Loading Product Editor...</span>
      </div>
    );
  }
    ...
    <div className="p-6 sm:p-10 space-y-6 max-w-4xl w-full mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="text-2xl font-serif text-charcoal-900">Edit Product & Color Variants</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 bg-white rounded-3xl border border-cream-300 shadow-soft space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-4">
          <div className="sm:col-span-8">
            ...
            <input
              ...
              placeholder="Handmade Product Title"
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2">
            ...
            {formData.thumbnail ? (
              ...
              <div className="relative">
                <input
                  ...
                  className="hidden"
                  id="product-image-file-upload-edit"
                />
                <label
                  htmlFor="product-image-file-upload-edit"
            ...
            <div className="relative">
              <input
                ...
                className="hidden"
                id="product-gallery-file-upload-edit"
              />
              <label
                htmlFor="product-gallery-file-upload-edit"
        ...
        <div className="p-5 bg-white rounded-2xl border-2 border-brand-200 shadow-xs space-y-4">
          ...
          <div className="space-y-4">
            {variants.map((v, idx) => (
              <div key={idx} className="p-4 bg-cream-50 rounded-2xl border border-cream-300 space-y-3">
                    ...
                    <div className="relative shrink-0">
                      ...
                      {v.imageUrl ? (
                        <div className="relative w-16 h-16 rounded-xl overflow-hidden bg-white border-2 border-brand-500 group shadow-xs">
                          ...
                          <label
                            htmlFor={`variant-upload-edit-${idx}`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="199"
              className="w-full px-3.5 py-2.5 bg-cream-50 border border-cream-300 rounded-xl text-xs outline-none focus:border-brand-600 text-charcoal-900 font-bold"
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-charcoal-800 mb-1">Stock Count *</label>
            <input
              type="number"
              required
              name="stock"
              ...
              onChange={handleChange}
              className="w-full px-3.5 py-2.5 bg-cream-50 border border-cream-300 rounded-xl text-xs outline-none focus:border-brand-600 text-charcoal-900 font-semibold text-emerald-800"
            />
          </div>
        </div>
        ...
        {/* Customization Settings & Fields Builder */}
        <div className="p-5 bg-cream-50 rounded-2xl border border-cream-300 space-y-4">
          <div className="flex items-center justify-between border-b border-cream-200 pb-3">
            <label className="flex items-center gap-2.5 text-xs font-bold text-charcoal-900 cursor-pointer">
              <input
                type="checkbox"
                name="customizable"
              ...
              <span className="flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-terracotta-600" /> Enable Product Customization Options
              </span>
            </label>
            ...
            {formData.customizable && (
              <button
                ...
                className="px-3 py-1.5 bg-brand-100 hover:bg-brand-200 text-brand-900 rounded-xl text-xs font-semibold flex items-center gap-1 transition-colors"
              >
                <Plus className="w-3.5 h-3.5 text-terracotta-600" /> Add Custom Field
              </button>
            )}
          </div>
            ...
            customizations.length > 0 ? (
              <div className="space-y-3">
                {customizations.map((c, idx) => (
                  <div key={idx} className="flex items-center gap-3 bg-white p-3 rounded-xl border border-cream-300">
                    <input
                      type="text"
                      placeholder="Field Label e.g. Custom Name / Initials"
                      value={c.label}
                      onChange={(e) => {
                      ...
                      className="flex-1 px-3 py-2 bg-cream-50 border border-cream-300 rounded-lg text-xs font-medium outline-none focus:border-brand-600 text-charcoal-900"
                    />
                    <button
                      type="button"
              ...
              <p className="text-xs text-charcoal-500 italic text-center py-2">
                No custom input fields added yet. Click "+ Add Custom Field" above to add text or photo inputs.
              </p>
            )
          ) : (
            <p className="text-xs text-charcoal-500 italic">
              Customization is disabled for this product. Unchecking this removes customization options on the storefront.
            </p>
          )}
        </div>
        ...
        <button
          ...
          className="w-full py-4 px-6 bg-brand-800 hover:bg-brand-900 text-cream-100 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2 shadow-float cursor-pointer"
        >
